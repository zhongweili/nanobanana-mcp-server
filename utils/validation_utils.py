...
def validate_positive_integer(value: Any, name: str, min_value: int = 1, max_value: Optional[int] = None) -> None:
...
def validate_string_length(value: str, name: str, min_length: int = 0, max_length: Optional[int] = None) -> None:
...
def validate_json_structure(data: Any, required_fields: List[str], optional_fields: Optional[List[str]] = None) -> None:
...
def validate_aspect_ratio(width: int, height: int, min_ratio: float = 0.1, max_ratio: float = 10.0) -> None:
    ...
    if ratio < min_ratio or ratio > max_ratio:
        raise ValidationError(f"Aspect ratio {ratio:.2f} is outside valid range ({min_ratio}-{max_ratio})")
...
def validate_content_type(content_type: str, allowed_types: List[str]) -> None:
    ...
    if main_type not in [t.lower() for t in allowed_types]:
        raise ValidationError(f"Content type '{main_type}' not allowed. Allowed types: {', '.join(allowed_types)}")
...
def validate_timeout_seconds(timeout: Union[int, float], min_timeout: float = 0.1, max_timeout: float = 300.0) -> None:
