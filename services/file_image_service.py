...
class FileImageService:
    ...
    def __init__(self, gemini_client: GeminiClient, gemini_config: GeminiConfig, server_config: ServerConfig):
    ...
    def generate_images(
        ...
        with ProgressContext("image_generation", f"Generating {n} image(s)...", {"prompt": prompt[:100], "count": n}) as progress:
            ...
            progress.update(100, f"Generated and saved {len(file_metadata)} image(s) to {self.output_dir}")
        ...
        with ProgressContext("image_editing", "Editing image...", {"instruction": instruction[:100]}) as progress:
            ...
            try:
                ...
                for i, image_bytes in enumerate(image_bytes_list):
                    progress.update(70 + (i * 20 // len(image_bytes_list)), f"Saving result {i+1}/{len(image_bytes_list)}...")
                    ...
                    self.logger.info(f"Saved edited image to {full_path} ({len(image_bytes)} bytes)")
                
                progress.update(100, f"Successfully edited and saved {len(thumbnail_images)} image(s)")
                return thumbnail_images, file_metadata
    ...
    def get_output_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recent_images": [str(f.name) for f in sorted(image_files, key=lambda x: x.stat().st_mtime, reverse=True)[:5]]
            }
        except Exception as e:
